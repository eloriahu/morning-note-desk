...
class CollectorLauncherTests(unittest.TestCase):
    ...
    def test_existing_local_project_preserves_config_and_priorities(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(Path(command[1]), project / "morning_note.py")
            self.assertEqual(Path(command[command.index("--config") + 1]), project / "config.json")
    ...
    def test_fresh_workspace_uses_bundled_defaults_and_empty_priorities(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(config_path.parent.parent, workspace / "morning-note-runs")
